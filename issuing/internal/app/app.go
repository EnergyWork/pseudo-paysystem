...
func (a *App) Run() *errs.Error {
	...
	// Use case
	useCase := usecase.New(repository, a.set)

	// NATS Server
	natsController := queue.New(a.set, useCase)
	natsServer, err := natsserver.New(a.set.NATS(), queue.Queue, natsController.NewRouter()) // this starts a server
	if err != nil {
		a.set.Log().Error(err)
	}
	a.set.Log().Info("The nats server is listening to an queue: %s", queue.Queue)
